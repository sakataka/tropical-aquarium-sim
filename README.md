...
起動するとフィッシュルームが表示され、並んだ水槽のガラスの中では実際の水景と魚が泳いでいます。水槽を選ぶとカメラが寄り、その水槽の画面へ入ります。「部屋に戻る」で引いて部屋へ戻ります。
...
`bun run verify:webview` はWebKit backendの `Bun.WebView` を使い、フィッシュルーム、水槽へ寄って入る操作、水槽ごとの魚と上限、4水景、照明、環境音、観賞モード、部屋へ戻る操作、3水槽の切り替え、再読込後の復元、v4からの移行、420px幅の表示を確認します。スクリーンショットは `tmp/webview/` に保存します。
