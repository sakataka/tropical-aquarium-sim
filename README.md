...
起動するとフィッシュルームが表示され、並んだ水槽のガラスの中では実際の水景と魚が泳いでいます。水槽を選ぶとカメラが寄り、そのまま水槽を画面いっぱいに眺める鑑賞モードへ入ります。水槽は部屋のガラス越しに見えていたのと同じ切り取り方で、余白を残して全体が映ります。ホイール・ピンチ・`+`/`-` キー・ダブルクリック・画面右下のボタンで近づき（`0` か「全体」で戻る）、近づいたときはドラッグや矢印キーで見回せます。鑑賞モードの操作ボタンは、しばらく操作しないと消えます。「設定」で操作パネルを開き（狭い画面では下から）、「閉じて眺める」で鑑賞モードへ戻ります。「部屋に戻る」か Esc で引いてフィッシュルームへ戻ります（設定パネルを開いているときの Esc はパネルを閉じます）。
