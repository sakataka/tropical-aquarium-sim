...
export class FishBody {
  ...
  private amplitude = 0.12;
  private yaw: number;
  private yawVelocity = 0;
  private targetYaw: number;
  private sinceTurnSec = MIN_TURN_INTERVAL_SEC;
  private pitch = 0;
  ...
  constructor(texture: Texture, species: FishSpeciesDefinition, fish: FishInstance) {
    ...
    this.phase = (Math.abs(fish.seed) % 628) / 100;
    this.yaw = fish.facing === 1 ? Math.PI : 0;
  ...
  private updateYaw(fish: FishInstance, deltaSec: number) {
    this.sinceTurnSec += deltaSec;
    const vx = fish.velocity.x;
    const facingRight = this.targetYaw > Math.PI / 2;
    if (this.sinceTurnSec >= MIN_TURN_INTERVAL_SEC) {
    ...
    const accel = (this.targetYaw - this.yaw) * TURN_STIFFNESS - this.yawVelocity * TURN_DAMPING;
    this.yawVelocity += accel * deltaSec;
    this.yaw += this.yawVelocity * deltaSec;
  }
  ...
  private deform() {
    ...
    for (let column = 0; column < VERTICES_X; column += 1) {
      ...
      const wave = Math.sin(this.phase - u * waveCount * Math.PI * 2);
      const angle = this.amplitude * envelope * wave;
